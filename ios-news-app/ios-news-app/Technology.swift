...
class Technology: UITableViewController, IndicatorInfoProvider {
    ...
    override func viewDidLoad() {
        ...
        technologyArticlesTable.addSubview(tableRefreshControl)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        articleCell.articleDate?.text = technologyArticles[index].date
        articleCell.articleSection?.text = "| " + technologyArticles[index].section
    ...
    override func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        let share = UIAction(title: "Share with Twitter", image: UIImage(named: "twitter")) {_ in
            ...
            let shareURL = "https://twitter.com/intent/tweet?text=\(text)&url=\(self.technologyArticles[index].url)&hashtags=CSCI_571_NewsApp"
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let cell = technologyArticlesTable.cellForRow(at: indexPath as IndexPath)
    ...
    func displayArticleImage(_ row: Int, articleCell: TechnologyTableViewCell) {
        if(technologyArticles[row].imageURL != "undefined") {
            let url: String = (URL(string: technologyArticles[row].imageURL)?.absoluteString)!
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let guardianURL = "http://assafp-nodejs.us-east-1.elasticbeanstalk.com/guardian-technology"
        AF.request(guardianURL).responseJSON { response in
            switch response.result {
            case let .success(value):
                self.technologyArticles.removeAll()
                ...
                for (key, article) in guardianJSON {
                    ...
                    self.technologyArticles.append(Article(key: key, id: id, title: title, date: date, section: section, imageURL: img, description: "", url: url))
                }
                self.technologyArticlesTable.reloadData()
